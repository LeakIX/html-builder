...
impl Node {
    ...
    pub fn render_to(&self, output: &mut String) {
        match self {
            Node::Element(e) => e.render_to(output),
...
impl Element {
    ...
    pub fn find_all_elements(&self, tag_name: &str) -> Vec<&Element> {
        let mut results = Vec::new();
        self.collect_elements(tag_name, &mut results);
        results
    }

    fn collect_elements<'a>(&'a self, tag_name: &str, results: &mut Vec<&'a Element>) {
        for child in &self.children {
            if let Node::Element(e) = child {
                if e.tag_name.eq_ignore_ascii_case(tag_name) {
                    results.push(e);
    ...
    fn collect_by_class<'a>(&'a self, class_name: &str, results: &mut Vec<&'a Element>) {
