...
//! using the `html-builder` and `html-elements` crates.
//!
//! ## Syntax
//!
//! ```rust
...
impl ToTokens for Node {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        match self {
            Node::Element(elem) => elem.to_tokens(tokens),
            Node::Text(lit) => {
            ...
            Node::For(for_loop) => for_loop.to_tokens(tokens),
            Node::If(if_node) => if_node.to_tokens(tokens),
        }
    }
}
...
impl ToTokens for ElementNode {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        let attr_calls: Vec<_> = self.attrs.iter().map(|a| a.to_token_stream()).collect();

        if self.children.is_empty() {
            // No children - just create element with attributes
            tokens.extend(quote! {
                ::html_builder::typed::Element::<::html_elements::#tag_ident>::new()
                    #(#attr_calls)*
            });
        } else {
            ...
            for child in &self.children {
                match child {
                    Node::Element(elem) => {
                        ...
                        let child_attrs: Vec<_> =
                            elem.attrs.iter().map(|a| a.to_token_stream()).collect();

                        if elem.children.is_empty() {
                            child_tokens.extend(quote! {
                                .child::<::html_elements::#child_ident, _>(|e| e #(#child_attrs)*)
                            });
                        } else {
                            let nested = generate_children(&elem.children);
                            child_tokens.extend(quote! {
                                .child::<::html_elements::#child_ident, _>(|e| e #(#child_attrs)* #nested)
                            });
                        }
                    }
                    Node::Text(lit) => {
    ...
    for child in children {
        match child {
            Node::Element(elem) => {
                ...
                let child_attrs: Vec<_> = elem.attrs.iter().map(|a| a.to_token_stream()).collect();
...
impl ToTokens for Attribute {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        let method_name = match name_str.as_str() {
            "type_" => Ident::new("attr", name.span()),
            "for_" => Ident::new("attr", name.span()),
            "class" | "id" => name.clone(),
...
impl ToTokens for ForLoop {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        if let Some(Node::Element(elem)) = self.children.first() {
            ...
            tokens.extend(quote! {
                .children(#expr, |#pat, e: ::html_builder::typed::Element<::html_elements::#child_ident>| {
...
fn to_pascal_case(s: &str) -> String {
    ...
    match result.as_str() {
        ...
        "Nav" => "Nav".to_string(),
        "Svg" => "Svg".to_string(),
        // Option is special - html-elements uses Option_
